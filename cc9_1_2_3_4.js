...
function convertToCamelCase(sentence) {
  ...
  for (const word of words) {
    ...
    const upperCaseSecondWordFirstLetter = secondWord[0].toUpperCase();
    const upperCaseSecondWord =
      upperCaseSecondWordFirstLetter + secondWord.slice(1);
    const camelCase = firstWord + upperCaseSecondWord;
    console.log(camelCase);
  }
}

// convertToCamelCase('test_word2 \n test2_word2 \n test3_word2');

const textAreaDOM = document.getElementById('camel');
const convertButton = document.getElementById('convert');

convertButton.addEventListener('click', function () {
  const variableNames = textAreaDOM.value;
  convertToCamelCase(variableNames);
});
